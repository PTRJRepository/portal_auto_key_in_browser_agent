...
def test_loosefruit_staging_url_accepts_full_user_route():
    ...
    assert url == "http://localhost:3001/upah/staging-comparison?periode=2026-05"
...
def test_normalize_and_filter_eligible_loosefruit_rows():
    comparison = normalize_loosefruit_staging_payload(
        {
            "data": {
                ...
                "rows": [
                    ...
                    {"emp_code": "B0001", "estate": "P1A", "gang": "A1H", "staging_brondol": 5, "plantware_brondol": 1, "selisih": 4},
    ...
    assert [row.emp_code for row in eligible] == ["A0001"]
