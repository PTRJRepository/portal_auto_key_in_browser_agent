...
def loosefruit_staging_comparison_url(base_or_url: str, periode: str) -> str:
    source = (base_or_url or "http://localhost:8002").strip().rstrip("/")
    query = urlencode({"periode": periode})
    if source.endswith("/upah/staging-comparison") or source.endswith("/backend/upah/api/staging/staging-comparison"):
...
def normalize_loosefruit_staging_row(row: dict[str, Any]) -> LoosefruitStagingRow:
    return LoosefruitStagingRow(
        ...
        gang=str(row.get("gang") or row.get("Gang") or "").strip().upper(),
        ...
        estate=str(row.get("estate") or row.get("loc_code") or row.get("LocCode") or "").strip().upper(),
        staging_brondol=numeric_value(row.get("staging_brondol")),
        plantware_brondol=numeric_value(row.get("plantware_brondol")),
        selisih=numeric_value(row.get("selisih")),
    )
...
def eligible_loosefruit_rows(rows: list[LoosefruitStagingRow], estate: str | None = None) -> list[LoosefruitStagingRow]:
    ...
    return [
        row for row in rows
        if row.selisih > 0
        and row.emp_code.startswith("A")
        and (not estate_filter or row.estate == estate_filter)
    ]


def fetch_loosefruit_staging_comparison(base_or_url: str, periode: str, timeout_seconds: int = 30) -> LoosefruitStagingComparison:
    url = loosefruit_staging_comparison_url(base_or_url, periode)
