...
Monthly allowance AD Code forms can be block-based or vehicle-based after the AD Code postback. See `docs/MONTHLY-ALLOWANCE-DOM-PATTERNS.md` before changing runner form-fill logic for block/sub-block or vehicle expense rows. In grouped premium metadata, `subblok` means block-based; `nomor_kendaraan`/`vehicle_code` means vehicle-based, and metadata `expense_code` maps to the Plantware Vehicle Expense Code.
