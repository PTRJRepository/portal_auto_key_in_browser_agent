...
export async function runGetSession(payload: RunPayload, emit: EmitEvent): Promise<RunResult> {
  ...
  const session = new BrowserSession({ headless: payload.headless, freshLoginFirst: true });
  emit({ event: "session.get.started", message: "Getting fresh Plantware session..." });
  try {
    await session.start();
    emit({ event: "session.get.completed", message: "Fresh session saved and ready; no records attempted", session_reused: false, attempted_rows: 0, inserted_rows: 0 });
...
export async function runTestSession(payload: RunPayload, emit: EmitEvent): Promise<RunResult> {
  const started_at = new Date().toISOString();
  const session = new BrowserSession({ headless: payload.headless, freshLoginFirst: false });
  emit({ event: "session.test.started", message: "Testing saved Plantware session..." });
  try {
    await session.start();
    emit({ event: "session.test.completed", message: session.sessionReused ? "Saved session is valid; no records attempted" : "Saved session was missing/invalid; fresh session created; no records attempted", session_reused: session.sessionReused, attempted_rows: 0, inserted_rows: 0 });
