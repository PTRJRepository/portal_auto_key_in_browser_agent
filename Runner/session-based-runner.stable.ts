/**
 * SPSI Session-Based Runner (STABLE VERSION)
 * Reuses authenticated browser sessions for faster execution.
 * Stores session state in filesystem for persistence across runs.
 *
 * OPTIMIZATION NOTES:
 * - Session storage state must be passed as object, NOT file path
 * - Session directory path must be relative to project root: ../../../Runner/spsi_input/sessions
 * - Session timeout: 4 hours (for ASP.NET sessions)
 * - Cookie domain: plantwarep3
 */
